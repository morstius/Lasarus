...
const LONG g_lApplicationWidth = 1280;
const LONG g_lApplicationHeight = 720;
LPCSTR g_strWindowClassName = "Lasarus";
LPCSTR g_strWindowName = "Lasarus";
HWND g_hwndWindowHandle = 0;

const BOOL g_bVSync = FALSE;
...
int InitApplication(HINSTANCE hInstance, int cmdShow)
{
	...
	wndClass.lpszClassName = g_strWindowClassName;
	...
	RECT windowRect = { 0, 0, g_lApplicationWidth, g_lApplicationHeight };
	AdjustWindowRect(&windowRect, WS_OVERLAPPEDWINDOW, FALSE);

	g_hwndWindowHandle = CreateWindowA(g_strWindowClassName, g_strWindowName,
		WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT,
		windowRect.right - windowRect.left,
		windowRect.bottom - windowRect.top,
		nullptr, nullptr, hInstance, nullptr);

	if (!g_hwndWindowHandle)
	{
		return -1;
	}

	ShowWindow(g_hwndWindowHandle, cmdShow);
	UpdateWindow(g_hwndWindowHandle);
...
int InitDirectX(HINSTANCE hInstance, BOOL vSync)
{
	...
	assert(g_hwndWindowHandle != 0);

	RECT clientRect;
	GetClientRect(g_hwndWindowHandle, &clientRect);
	...
	swapChainDesc.OutputWindow = g_hwndWindowHandle;
...
#if _DEBUG
	LPCWSTR vertexShader = L"../shaders/SimpleVertexShader.hlsl";
#else
	LPCWSTR vertexShader = L"../shaders/SimpleVertexShader.hlsl";
#endif

	hr = CompileShaderFromFile(vertexShader, "SimpleVertexShader", "vs_4_0", &vertexShaderBlob);
	if (FAILED(hr))
	{
		return false;
	}
...
#if _DEBUG
	LPCWSTR pixelShader = L"../shaders/SimplePixelShader.hlsl";
#else
	LPCWSTR pixelShader = L"../shaders/SimplePixelShader.hlsl";
#endif

	hr = CompileShaderFromFile(pixelShader, "SimplePixelShader", "ps_4_0", &pixelShaderBlob);
	if (FAILED(hr))
	{
		return false;
	}
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE prevInstance, LPWSTR cmdLine, int cmdShow)
{
	...
	if (InitDirectX(hInstance, g_bVSync) != 0)
	{
